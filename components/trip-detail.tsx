    ...
    <div className="container mx-auto px-4 py-8 space-y-8">
      {trip.imageUrl && (
        <div className="w-full h-72 md:h-96 overflow-hidden rounded-xl shadow-lg relative">
          <Image
            ...
            priority
            unoptimized
            onLoadingComplete={() => console.log('Image loaded successfully:', trip.imageUrl)}
            onError={(e) => console.error('Next.js Image Error for URL:', trip.imageUrl, e)}
          />
        </div>
      )}
      ...
      <div className="bg-white p-6 shadow rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center">
        ...
        <div className="mt-4 md:mt-0">
          <Link href={`/trips/${trip.id}/itinrerary/new`}>
            <Button>
              {' '}
      ...
      <div className="bg-white p-6 shadow rounded-lg">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <div className="grid md:grid-cols-2">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {trip.startDate.toLocaleDateString()} - {trip.endDate.toLocaleDateString()}
                        <br />
                        {`${Math.round((trip.endDate.getTime() - trip.startDate.getTime()) / (1000 * 60 * 24 * 60))} days(s)`}
                      </p>
                    </div>
                  </div>
              ...
              {trip.locations.length === 0 && (
                <div className="text-center p-4">
                  <p> Add locations to see on the map.</p>
                  <Link href={`/trips/${trip.id}/itinrerary/new`}>
                    <Button>
                      {' '}
                      <Plus className="mr-2 h-5 w-5" /> Add Location
          ...
          <TabsContent value="itinerary" className="space-y-6">
            ...
            {trip.locations.length === 0 ? (
              <div className="text-center p-4">
                <p> Add locations to see on the itinrerary.</p>
                <Link href={`/trips/${trip.id}/itinrerary/new`}>
                  <Button>
                    {' '}
                    <Plus className="mr-2 h-5 w-5" /> Add Location
